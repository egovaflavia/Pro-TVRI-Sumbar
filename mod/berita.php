...
if($se == 'detil') {
 ...
 echo "";
  ...
  if($sqldetil->foto <> '') { $imgdetil = "<div id=\"foto-detil\" align=\"center\"><img src=\""._URLWEB."up/berita/".$sqldetil->foto."\" class=\"img-konten\" alt=\"$sqldetil->judul\"></div>"; } else { $imgdetil = ""; }
...
<div class="tgl-detil"><?=$mediadetil->nama;?> &bull; <?=$wldetil->nama;?><img src="<?php echo _URLWEB;?>img/jam10.png" width="10" height="10" alt="" border="0" style="padding:0px 4px 0px 10px;" /><?php $tanggal->contanggalx(substr($sqldetil->tgl_jam,8,2),substr($sqldetil->tgl_jam,5,2),substr($sqldetil->tgl_jam,0,4)); echo " - ".substr($sqldetil->tgl_jam,11,5)." WIB";?></div>
<div class="judul-detil"><?=$sqldetil->judul;?></div>
<?php if($sqldetil->subjudul <> '') { echo "<div class=\"judul-sub\">$sqldetil->subjudul</div>"; } ?>
...
<!-- box detil -->
        ...
        while($rowt = mysql_fetch_object($tsqldetil))
        {
          ...
          if($rowt->foto <> '') { $imgt = _URLWEB."up/berita/thumb_".$rowt->foto; } else { $imgt = _URLWEB."img/thumb_no-img-berita-pps.jpg"; }
        
        ?>


        <li><div class="kot-sub"><img src="<?=$imgt;?>" width="192" height="120"><p style="padding: 0px 8px 8px 8px;" align="center"><a href="<?php echo _URLWEB."berita/detil/$rowt->id_br/".strtolower(str_replace(" ","-",$judult1)).".html";?>"><?=substr($rowt->judul,0,45);?>..</a></p></div></li>
...
<?php
	if(isset($_POST['cari']) or isset($_GET['cari'])) {
		if(isset($_POST['cari'])) { $katacari = $_POST['cari']; } 
		if(isset($_GET['cari'])) { $katacari = $_GET['cari']; } 
...
<?php
    ...
    <?php
		...
		while($rowqb=mysql_fetch_object($sql)) {
          ...
          if($rowqb->foto <> '') { $imgqb = _URLWEB."up/berita/".$rowqb->cfoto; } else { $imgqb = _URLWEB."img/thumb_no-img-berita-bs.jpg"; }
		...
		<p class="tgl-berita-home"><a href="<?php echo _URLWEB;?>berita"><b><?=$kanalqb->nama;?></b></a><img src="<?php echo _URLWEB;?>img/jam10.png" width="10" height="10" alt="" border="0" style="padding:0px 4px 0px 10px;" /><?php $tanggal->contanggalx(substr($rowqb->tgl_jam,8,2),substr($rowqb->tgl_jam,5,2),substr($rowqb->tgl_jam,0,4)); echo " ".substr($rowqb->tgl_jam,11,5)." WIB"; ?></p>
